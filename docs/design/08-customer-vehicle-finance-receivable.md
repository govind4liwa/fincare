...
FinanceAgreement (direction = LOAN_PAYABLE | FINANCE_RECEIVABLE)
 ...
 │    └── ChargeNoteLine
 └── SettlementAllocation (receipt/credit/set-off → instalment or charge note)
```
...
| status | choice | `draft → active → settled → cancelled` |
| journal_entry | FK ledger.JournalEntry, null | activation JE if policy posts one (§7.1) |

CHECK constraints: receivable ⇒ customer + instalment_control_account set;
principal > 0 on activation. Unique: (entity, agreement_no) when assigned.
...
### 5.4 `SettlementAllocation` (receipt → instalment/charge)
...
| target_id | UUID |
| amount_allocated | > 0 always; reversals are **new contra rows** with `source_type=reversal` linking `reverses` FK — never edits or negative amounts |
| allocation_date | |

Service invariants: Σ allocations per target ≤ target total; Σ allocations per
source ≤ source amount; both enforced under `select_for_update` on the target.
Unallocated receipt remainder stays on the customer as an advance (visible in
the statement; allocatable later).
...
- `overdue`: **derived flag** (`due_date < today` and not fully paid), computed
  in queryset annotations / serializer — deliberately not a stored status, so
  the state machine has no time-driven transitions to run.
...
Accrual basis: interest income lands at due date, per the approved schedule.
A1 therefore always equals the schedule's outstanding principal for posted
rows — the "Outstanding principal 103,611.69 after 12" figure is a GL balance,
not a spreadsheet cell.

### 7.3 Receipt

Standard receipt voucher (vouchers app): `DR Bank / CR A2 (party=customer)`,
then `SettlementAllocation` rows spread it across instalments and charge notes
...
4. Over-allocation impossible (Σ per target ≤ total; Σ per source ≤ source),
   enforced under row locks; property-based test hammers this concurrently.
...
6. Remaining balance per instalment/charge = `total − amount_allocated`,
   denormalized and recomputed by the service on every change.
...
- **Opening balance** = Σ(debit−credit) on A2 for the customer before the
  from-date (plus `ar.Customer.opening_balance` when migrated openings exist).
...
- Charge-note recharges commonly *are* taxable (reimbursement vs disbursement
  ...
  app unchanged.
- Interest income timing (accrual at due date per §7.2) gives Corporate Tax a
  clean accrual base; cash-basis reporting derives from receipts as elsewhere
  in FinCare.

## 12. API surface (DRF, `/api/financing/`)
...
7. Migration (Slice B): row counts, checksums of money columns, JE links
   preserved, fleet endpoints still green through delegation.
...
## 17. Open policy decisions (need your call, not code)

1. **Vehicle derecognition at activation** — installment sale (derecognize,
   book gain/loss) vs retain until title transfer. Affects §7.1 mapping only.
2. **VAT classification** of finance income and each recharge item type
   (advisory; configured per §11).
3. **Default overdue grace** (days after due date before the statement flags
   arrears) — cosmetic, but fix it for aging buckets.
4. **Early settlement policy** — interest rebate formula on foreclosure
   (workbook is silent; propose schedule re-version with a settlement
   instalment; deferred until a real case).
