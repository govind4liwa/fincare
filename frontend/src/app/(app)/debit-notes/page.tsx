...
import { listDebitNotes, type DebitNote, type NoteStatus } from "@/lib/debit-notes";
...
export default function DebitNotesPage() {
  ...
  useEffect(() => {
    let active = true;
    listDebitNotes({ entityId: selectedId })
      .then((data) => {
        if (active) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {loading ? (
        ...
        <p className="text-sm text-destructive">Couldn&apos;t load debit notes.</p>
      ) : notes.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center text-sm text-muted-foreground">
            No debit notes yet. A debit note reduces what you owe a supplier (returns, corrections) —
            create one with <span className="font-medium text-foreground">New debit note</span>.
          </CardContent>
        </Card>
      ) : (
        <Card>
                ...
                <tbody>
                  {notes.map((n) => (
                    <tr key={n.id} className="border-b border-border/60 last:border-0">
                      <td className="whitespace-nowrap px-4 py-2">{n.debit_note_date}</td>
                      <td className="whitespace-nowrap px-4 py-2 font-mono text-xs">
                        {n.debit_note_no || <span className="text-muted-foreground">(draft)</span>}
                      </td>
      ...
      {!loading && !error && notes.length > 0 && (
        <p className="text-xs text-muted-foreground">
          {notes.length} of {count} debit note{count === 1 ? "" : "s"}
        </p>
      )}
    </div>
  );
